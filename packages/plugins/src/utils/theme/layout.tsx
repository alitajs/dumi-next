...
const useCurrentMenu = (
  ...
  const [menu, setMenu] = useState<IMenu['locale']['path']>(
...
const useCurrentBase = (
  ...
  const handler = (
    ...args: [string, IThemeContext['config']['locales'], IRoute]
  ) => {
 ...
 * outer theme layout
 */
const OuterLayout: React.FC<IOuterLayoutProps & IRouteComponentProps> = (
  props,
) => {
