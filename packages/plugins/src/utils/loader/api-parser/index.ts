...
export default (
  filePath: string,
  {
    componentName,
    ...filterOpts
  }: IStaticPropFilter & { componentName?: string } = {},
) => {
    ...
    default:
      localFilter = (
        ...
        (prop, component) => {
          ...
          return (
            builtinFilter(prop, component) && extraFilter(prop, mergedOpts)
          );
        }
  ...
  if (!definitions) {
    ...
    parser
      ...
      .forEach((item) => {
        ...
        const exportName = isDefaultRegExp.test(item.displayName)
          ? 'default'
          : item.displayName;
            ...
            switch (field) {
              ...
              case 'description':
                ...
                if (localeDescReg.test(prop.description)) {
                  ...
                  groups?.forEach((str) => {
                    const [, locale, content] = str.match(
                      /^(\.[\w-]+)?\s*([^]*)$/,
                    );
