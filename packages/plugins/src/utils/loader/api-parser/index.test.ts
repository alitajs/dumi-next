...
function assertResult(filename, extraProperties?) {
  expect(
    winEOL(
      JSON.stringify(
        parser(path.join(rawPath, filename), extraProperties),
        null,
        2,
      ),
    ),
    ...
    winEOL(
      fs
        .readFileSync(
          path.join(expectPath, `${path.basename(filename, '.tsx')}.json`),
          'utf8',
        )
        .toString(),
    ),
...
describe('api parser', () => {
  ...
  it('should guess component name correctly', () => {
    expect(parser(path.join(rawPath, 'guess', 'FileName.tsx'))).toHaveProperty(
      'FileName',
    );
    expect(
