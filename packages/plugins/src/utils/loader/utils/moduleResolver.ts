...
export const getModuleResolvePath = ({
  ...
  if (
    /^[a-z]@/.test(sourcePath) &&
    getHostPkgPath(getPkgPathsFromPath(sourcePath).pkgName)
  ) {
  ...
  try {
    ...
    if (!silent) {
      ctx.umi?.logger.error(
        `[dumi]: cannot resolve module ${sourcePath} from ${basePath}`,
      );
    }
...
export const getModuleResolvePkg = ({
  ...
  if (modulePath && fs.existsSync(pkgPath)) {
    ...
    ctx.umi?.logger.error(
      `[dumi]: cannot find valid package.json for module ${modulePath}`,
    );
  }
