...
const isDev = () =>
  process.env.NODE_ENV === 'development' || process.env.TEST_WATCHER;
...
export const listenFileOnceChange = (
  filePath: string,
  listener: IWatcherItem['listeners'][0],
) => {
  if (isDev()) {
    ...
    const existingListenerIndex = watchers[filePath].listeners?.findIndex(
      (fn) =>
        (fn._identifier || listener._identifier) &&
        fn._identifier === listener._identifier,
    );
