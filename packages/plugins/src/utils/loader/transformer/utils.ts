...
export const encodeHoistImport = (resolvePath: string) =>
  `import ${HOIST_ID} from '!!dumi-raw-code-loader!${winPath(
    resolvePath,
  )}?dumi-raw-code'`;
export const decodeHoistImport = (str: string, id: string) =>
  str.replace(new RegExp(HOIST_ID.replace(/\^/g, '\\^'), 'g'), id);
export const isHoistImport = (str: string) =>
  str.startsWith(`import ${HOIST_ID} from`);
...
export const isEncodeImport = (str: string) =>
  str.startsWith(`(await import(${CHUNK_ID}`);
export const decodeImportRequireWithAutoDynamic = (
  str: string,
  chunkName: string,
) =>
  isEncodeImport(str)
