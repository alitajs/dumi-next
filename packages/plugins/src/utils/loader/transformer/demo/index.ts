...
export { default as getDepsForDemo, getCSSForDep } from './dependencies';
  ...
  traverse(code.ast, {
    VariableDeclaration(callPath) {
      ...
      if (
        ...
        types.isCallExpression(
          callPathNode.declarations[0].init.arguments[0],
        ) &&
        types.isIdentifier(
          callPathNode.declarations[0].init.arguments[0].callee,
        ) &&
        callPathNode.declarations[0].init.arguments[0].callee.name ===
          'require' &&
        types.isStringLiteral(
          callPathNode.declarations[0].init.arguments[0].arguments[0],
        ) &&
        callPathNode.declarations[0].init.arguments[0].arguments[0].value ===
          'react'
      ) {
    ...
    CallExpression(callPath) {
      ...
      if (
        ...
        const isHeaderHelpers =
          /@babel\/runtime\/helpers\/(interopRequireWildcard|interopRequireDefault)$/.test(
  ...
  if (!reactVar) {
    body.unshift(
      types.variableDeclaration('var', [
        types.variableDeclarator(types.identifier('React')),
      ]),
    );
  }
