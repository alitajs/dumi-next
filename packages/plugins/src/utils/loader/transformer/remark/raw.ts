...
function hasComplexProp(props: Record<string, any>) {
  return Object.values(props).some(
    (prop) =>
      ['object', 'function'].includes(typeof prop) || Array.isArray(prop),
  );
}
...
export default (): IDumiUnifiedTransformer => (ast) => {
  ...
  visit(ast, (node) => {
    ...
    if (typeof node.tagName === 'string' && /^[A-Z]/.test(node.tagName)) {
      node.tagName = `dumi-raw${node.tagName.replace(
        /[A-Z]/g,
        (s) => `-${s.toLowerCase()}`,
      )}`;
      ...
      node.value = node.value.replace(
        /(<\/?)([A-Z]\w+)/g,
        (_, prefix, tagName) => {
          return `${prefix}dumi-raw${tagName.replace(
            /[A-Z]/g,
            (s) => `-${s.toLowerCase()}`,
          )}`;
        },
    ...
    if (
      ...
      hasComplexProp(node.properties)
    ) {
