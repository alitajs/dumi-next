  ...
  return (ast, vFile) => {
    if (this.data('fileAbsPath')) {
      const filePath = slash(
        path.relative(process.cwd(), this.data('fileAbsPath')),
      );
      ...
      if (Number.isNaN(Number(vFile.data.updatedTime))) {
        vFile.data.updatedTime = Math.floor(
          fs.lstatSync(this.data('fileAbsPath')).mtimeMs,
        );
      }

      // try to find related component of this md
      if (
        /(?<!\/src)\/index(\.[\w-]+)?\.md/i.test(
          slash(this.data('fileAbsPath')),
        )
      ) {
        try {
          ...
          vFile.data.componentName = path.basename(
    ...
    visit<YamlNode>(ast, 'yaml', (node) => {
      ...
      if (data.features) {
        data.features.forEach((feat) => {
          if (feat.desc) {
            feat.desc = transformer.markdown(feat.desc, null, {
              type: 'html',
            }).content;
          }
        });
      ...
      if (data.hero?.desc) {
        data.hero.desc = transformer.markdown(data.hero.desc, null, {
          type: 'html',
        }).content;
      }
      ...
      if (data.footer) {
        data.footer = transformer.markdown(data.footer, null, {
          type: 'html',
        }).content;
      }
      ...
      ctx.umi.applyPlugins({
        ...
        args: {
          ...
          _sourcePath: path.join(
            path.dirname(this.data('fileAbsPath')),
            'index.tsx',
          ),
