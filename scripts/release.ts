...
(async () => {
  ...
  examples.forEach((example) => {
    ...
    pkg.dependencies ||= {};
    if (pkg.dependencies['dumi']) pkg.dependencies['dumi'] = version;
    if (pkg.dependencies['@dumi/plugins'])
      pkg.dependencies['@dumi/plugins'] = version;
    delete pkg.version;
    fs.writeFileSync(
  ...
  await $`cd packages/dumi && npm publish --tag ${tag}`;
  logger.info(`+ dumi`);
  $.verbose = true;
